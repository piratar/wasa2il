    ...
    def process_request(self, request):
        ...
        try:
            ...
            if 'polity_id' in match.kwargs:
                ...
                global_vars[
                    'polity'
                ] = polity = Polity.objects.prefetch_related(
                    'members', 'officers', 'wranglers'
                ).get(
                    id=polity_id
                )

                if not request.user.is_anonymous:
                    global_vars['user_is_member'] = (
                        request.user in polity.members.all()
                    )
                    global_vars['user_is_officer'] = (
                        request.user in polity.officers.all()
                    )
                    # Officers are automatically wranglers.
                    if global_vars['user_is_officer']:
                        global_vars['user_is_wrangler'] = True
                    else:
                        global_vars['user_is_wrangler'] = (
                            request.user in polity.wranglers.all()
                        )
        except:
        ...
        if hasattr(settings, 'AUTO_LOGOUT_DELAY'):
            ...
            if not request.user.is_authenticated:
                ...
                if (
                    ...
                    request.session['last_visit'] = now.strftime(
                        '%Y-%m-%d %H:%M:%S'
                    )
            ...
            if 'last_visit' in request.session:
                last_visit = datetime.strptime(
                    request.session['last_visit'], '%Y-%m-%d %H:%M:%S'
                )
                if now - last_visit > timedelta(
    ...
    def process_request(self, request):
        ...
        if settings.SAML['URL']:  # Is SAML support enabled?
            ...
            if hasattr(settings, 'SAML_VERIFICATION_EXCLUDE_URL_PREFIX_LIST'):
                exclude_urls = (
                    settings.SAML_VERIFICATION_EXCLUDE_URL_PREFIX_LIST
                )
            else:
                exclude_urls = []
            ...
            verified = (
                request.user.userprofile.verified if logged_in else False
            )

            if logged_in and not verified and not path_ok:
                ctx = {'auth_url': settings.SAML['URL']}
                return render(
                    request, 'registration/verification_needed.html', ctx
                )
