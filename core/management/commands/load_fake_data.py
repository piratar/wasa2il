...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not options.get('full'):
            print()
            print(
                'NOTE: Creating small test data set, use --full for MOAR DATA.'
            )
            print()
        ...
        create_all = not (
            options.get('users', False)
            or options.get('topics', False)
            or options.get('elections', False)
            ...
            for u, email, name in userlist:
                if User.objects.filter(username=u).first() is None:
                    if len(u) == 1:
                        ...
                        print(
                            '   * Creating user "%s" with password "%s"'
                            % (u, u)
                        )
                    else:
                    ...
                    up.joined_org = now - timedelta(
                        hours=random.randint(0, 24 * 5)
                    )
                    up.save()
        ...
        for u, name, members, size in pollist:
            ...
            if new or options.get('users') or create_all:
                ...
                for m in set(
                    [m for m in members] + random.sample(users.keys(), size)
                ):
                    try:
            ...
            if options.get('elections') or create_all:
                ...
                for dc, dv in ((1, 2), (-1, 1), (-2, -1)):
                    ...
                    candidates = []
                    for cand in random.sample(
                        list(p.members.all()), candidatec
                    ):
            ...
            if new or options.get('documents') or create_all:
                ...
                aw = [
                    (m.username, max(20 - ul.index(m.username), 1))
                ...
                for docn in range(0, size):
                    ...
                    for version in range(0, random.randint(1, 3)):
                        text = '%s\n%s' % (text, text)
                        docc = DocumentContent(
                            document=doc, user=author, text=text
                        )
