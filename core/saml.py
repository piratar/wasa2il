...
def authenticate(input_xml, ca_pem_file):
    ...
    signed_xml = (
        XMLVerifier()
        .verify(input_xml, require_x509=True, ca_pem_file=ca_pem_file)
        .signed_xml
    )
    ...
    conds_xml = assertion.find(
        './{urn:oasis:names:tc:SAML:2.0:assertion}Conditions'
    )
    if not conds_xml:
        raise SamlException(
            'Could not find valid conditions statement. This is required.'
        )

    # @test: Verify audience.
    audience = conds_xml.find(
        "{urn:oasis:names:tc:SAML:2.0:assertion}AudienceRestriction/{urn:oasis:names:tc:SAML:2.0:assertion}Audience"
    ).text
    ...
    #   first 19 characters.
    time_limit_lower = datetime.strptime(
        conds_xml.attrib['NotBefore'][:19], '%Y-%m-%dT%H:%M:%S'
    )
    time_limit_upper = datetime.strptime(
        conds_xml.attrib['NotOnOrAfter'][:19], '%Y-%m-%dT%H:%M:%S'
    )
    now = datetime.now()
